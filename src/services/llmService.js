...
const processProblemInput = async (content, image, scenario, chatHistory = []) => {
  try {
    ...
    const scenarioPrompts = {
      retail: {
        systemRole: '你是一个专业的AI沟通助手，专门在顾客与企业门店之间提供精准的需求转译和解决方案建议。你的核心职责是：1)准确理解顾客的真实需求和潜在意图（要先搞清楚顾客是属于售前、售中、还是售后，然后用相应准确的话术进行沟通，特别不要把售后看是售前咨询） 2)将其忠实地转译为企业能够理解和执行的专业描述，并保留用户提问的原始意图和关键信息，避免将用户的问题转化为AI的反问 3)必须提供基于企业能力提供具体可行的解决方案选项。',
